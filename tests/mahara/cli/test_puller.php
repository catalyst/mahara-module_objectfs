...
$manipulator = new puller($filesystem, $config, $logger);
...
foreach ($candidatehashes as $candidatehash) {
    ...
    if (!empty($candidatehash->contenthash)) {
        ...
        $newlocation = $filesystem->copy_object_from_external_to_local_by_hash($candidatehash->contenthash, $candidatehash->filesize);
